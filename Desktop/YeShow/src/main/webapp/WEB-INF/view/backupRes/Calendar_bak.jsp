								...
								<c:if test="${ta.tablenum eq t.restablenum }"> 
									<div class="btn-group">
			              				...
			              				<c:if test="${t.resmin==0 }">
			              					...
			              					<!-- 예약이 되어있다면 표시하기 -->
			              					<!-- reserv예약된 날빠 보여주기 => re.resdate 
			              						   이전에 클릭해 예약하고 싶은 날짜 =>fresdate
			              						 tarr2예약된 시간 보여주기 => ta.yetrestime -->
			              					
			              					<!-- 예약된 테이블의 예약 날짜를 보기위한 for <c:forEach var="re" items="${reserv }">-->	 
			              					<!-- 예약된 테이블의 예약 시간를 보기위한 for <c:forEach var="tar2" items="${tarr2 }">-->
				              				 <a id="my">${t.restime }:${t.resmin }0</a>
					              				<c:forEach var="re" items="${reserv }">
					              					<c:forEach var="tar2" items="${tarr2 }">
								              			<c:if test="${re.resdate eq fresdate and tar2.yetrestime eq t.restime}"> 
									              				<script type="text/javascript">
									              				$('#my').addClass('btn btn-danger');
									              				</script>
									              				
								              			</c:if>
								              			<c:if test="${re.resdate ne fresdate}"> 
								              			<script type="text/javascript">
								              			$('#my').addClass('btn btn-default');
