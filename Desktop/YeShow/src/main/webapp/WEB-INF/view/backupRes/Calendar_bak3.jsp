				 ...
				 <div class="row">
					...
					<div class="form-group">  
						 <div class="col-md-3 text-left">
							<h4>예약 날짜 설정</h4><input type="date" class="form-control" id="resdate" name="resdate" value="${fresdate }">
					...
					<!-- 테이블 정보 보여주는 for -->
					 <c:forEach var="ta" items="${table }" varStatus="i">
						<div class="col-md-12">
							...
							<!-- 시간을 보여주는 for -->
							<c:forEach var="t" items="${tarr }" begin="0" end="4"> 
							${ta.tablenum}eq ${t.restablenum }
								<!-- 등록된 테이블과 각 시간을 표현하는 테이블의 넘버가 같을 떄-->
										<c:if test="${ta.tablenum eq t.restablenum }"> 	

								<!-- JOIN 테이블과 예약 조인, 예약된 테이블 번호, 시간, 날자 가져오기 -->
								<c:forEach var="ta2" items="${table2 }">
											...
											<div class="btn-group">
					              				...
					              				<!-- 분이 없을때 -->
					              				<!-- 날짜가 같다 -->
					              				<c:if test="${ta2.resdate eq fresdate}"> 
					              						<!-- 테이블이 예약테이블과 같을 대 -->
											           <c:if test="${ta.tablenum eq ta2.tablenum}">
											              		<!-- 예약 시간이 같다 -->
											              		 <c:if test="${t.restime eq ta2.restime }">	 
										      ...
										      <!-- 날짜가 같지 않을때  -->
										    <c:if test="${ta2.resdate ne fresdate}"> 
										              	<a	class="btn btn-default" onclick="reservation('${t.restime }:${t.resmin }0','${t.restablenum }')">${t.restime }:${t.resmin }0</a>
